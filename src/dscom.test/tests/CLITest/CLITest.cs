...
public class CLITest : CLITestBase
{
    ...
    public void CallWithoutCommandOrOption_ExitCodeIs1AndStdOutIsHelpStringAndStdErrIsUsed()
    {
        ...
        Assert.Contains(Strings.ErrorNoCommandOrOptions, result.StdErr.Trim());
    ...
    public void CallWithoutCommandABC_ExitCodeIs1AndStdOutIsHelpStringAndStdErrIsUsed()
    {
        ...
        Assert.Equal(1, result.ExitCode);
        Assert.Contains(Strings.ErrorNoCommandOrOptions, result.StdErr.Trim());
        Assert.Contains(Strings.FormatErrorUnrecognizedCommandOrArgument("ABC"), result.StdErr.Trim());
    }
    ...
    public void TlbExportAndDemoAssemblyAndCallWithTlbDump_ExitCodeIs0AndTlbIsAvailableAndValid()
    {
        ...
        // Current GitHub Action Runner with windows-2022 image seems to have applied
        // a patch for the following issue, so workaround is not required anymore
