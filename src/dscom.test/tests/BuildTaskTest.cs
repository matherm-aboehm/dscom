...
public class BuildTaskTest : BaseTest
{
    ...
    public void TestAssemblyFileReferencesItemSpecCheckFail()
    {
        ...
        Assert.True(task.Execute());
        Assert.False(task.Log.HasLoggedErrors);
        Assert.NotEqual(0, ((BuildEngineStub)task.BuildEngine).NumberOfWarningLogEvents); // "At least one warning should be present"
    }
    ...
    public void TestTypeLibraryReferencesHintPathCheckSuccess()
    {
        ...
        Assert.True(task.Execute());
        Assert.False(task.Log.HasLoggedErrors);
        Assert.Equal(5, ((BuildEngineStub)task.BuildEngine).NumberOfWarningLogEvents); // "Five warnings should be present, since TLBs cannot be referenced using HintPath"
    }
    ...
    public void TestypeLibraryReferencesHybridCheckSuccess()
    {
        ...
        Assert.True(task.Execute());
        Assert.False(task.Log.HasLoggedErrors);
        Assert.Equal(2, ((BuildEngineStub)task.BuildEngine).NumberOfWarningLogEvents); // "Two warnings should be present, since TLBs cannot be referenced using HintPath"
    }
