...
public class BuildTaskTest : BaseTest
{
...
#endif
        return Path.Combine(workdir.FullName, "src", "dscom.demo", projectName, "bin", configuration, targetFramework, fileName);
    }
    ...
    [InlineData(true, "assembly1", "dSPACE.Runtime.InteropServices.DemoAssembly1.dll")]
    ...
    public void TestAssemblyFileReferencesItemSpecCheckFail()
    {
        ...
        task.Execute().Should().BeTrue();
        task.Log.HasLoggedErrors.Should().BeFalse();
        task.BuildEngine.As<BuildEngineStub>().NumberOfWarningLogEvents.Should().NotBe(0, "At least one warning should be present");
    }
