...
public class RegistrationServices : IRegistrationServices
{
    ...
    internal static bool TypeRequiresRegistrationHelper(Type type)
    {
        ...
        // If the type does not have a public default constructor then is not creatable from COM so 
    ...
    public bool RegisterAssembly(Assembly assembly, bool registerCodeBase, ManagedCategoryAction preferredAction = ManagedCategoryAction.None)
    {
        ...
        {
            codeBase = new Uri(assembly.Location);
        }
    ...
    private static void RegisterValueType(Type type, string assemblyName, string assemblyVersion, Uri? codeBase, string runtimeVersion)
    {
        ...
        {
            recordVersionKey.SetValue(RegistryKeys.CodeBase, codeBase.ToString());
        }
    }
    ...
    private static void RegisterManagedType(Type type, string assemblyName, string assemblyVersion, Uri? codeBase, string runtimeVersion, ManagedCategoryAction preferredAction)
    {
        if (type.FullName is null)
        {
            throw new ArgumentException("Cannot register a type without a full name");
        ...
        if (null != comHostFile)
        {
            ...
            if (codeBase is not null)
            {
                inProcServerKey.SetValue(RegistryKeys.CodeBase, codeBase.ToString());
            }
            ...
            if (codeBase is not null)
            {
                versionSubKey.SetValue(RegistryKeys.CodeBase, codeBase.ToString());
            }
        }
    ...
    private static string? GetComHost(string assemblyName, Uri? codeBase)
    {
        ...
        var location = codeBase?.LocalPath;
        ...
        var extension = location != null ? Path.GetExtension(location) : ".dll";
        var fileBaseName = location != null ? Path.GetFileNameWithoutExtension(location) : assemblyName;
        var fileLocation = location != null ? Path.GetDirectoryName(location) : Environment.CurrentDirectory;
    ...
    private static void RegisterImportedComType(Type type, string assemblyName, string assemblyVersion, Uri? codeBase, string runtimeVersion)
    {
        if (type.FullName is null)
        {
            throw new ArgumentException("Cannot register a type without a full name");
    ...
    private static void RegisterPrimaryInteropAssembly(Assembly assembly, Uri? strAsmCodeBase, PrimaryInteropAssemblyAttribute attr)
    {
        ...
        {
            VersionSubKey.SetValue(RegistryKeys.PrimaryInteropAssemblyCodeBase, strAsmCodeBase!.ToString());
        }
    }
    ...
    private static bool CanWriteGlobalRegistry()
    {
        ...
        try
        {
            classesKey = Registry.LocalMachine.OpenSubKey(
                string.Join("\\", RegistryKeys.Software, RegistryKeys.Classes), true);
...
#pragma warning disable IDE0051 // Remove unused private member
    [Obsolete("For compatibility use CallUserDefinedRegistrationMethod instead")]
