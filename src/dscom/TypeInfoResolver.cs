...
internal sealed class TypeInfoResolver : ITypeLibCache
{
    ...
    private readonly string? _convertingAssemblyLocation;
    ...
    public TypeInfoResolver(WriterContext writerContext)
    {
        ...
        _convertingAssemblyLocation = string.IsNullOrEmpty(writerContext.Options.Assembly) ?
            null : Path.GetFullPath(writerContext.Options.Assembly);
    ...
    private ITypeInfo? ResolveTypeInfo(Type type, Guid guid)
    {
        ...
        var assemblyLocation = string.IsNullOrEmpty(assembly.Location) ?
            null : Path.GetFullPath(assembly.Location);
        var identifier = assembly.GetLibIdentifier(
            _convertingAssemblyLocation == assemblyLocation ?
